...
bool Board::isGuessValid(const QString &guess) {
    ...
    if (guess.size() != 5) {
        gameBar->setHintText(tr("Please enter 5 letters!"));
    // Check whether the guess is an existing word
    } else if (!Word::isValid(guess)) {
        gameBar->setHintText(tr("'%0' does not exist!").arg(guess));
...
bool Board::isHardValid(const QString &guess) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (prevColors[i] != Color::GRAY && !guess.contains(letter)) {
            gameBar->setHintText(tr("Missing letter '%0'!").arg(letter));
            return false;
        }
    }
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (prevColors[i] == Color::GREEN && guess[i] != letter) {
            gameBar->setHintText(tr("Letter #%0 must be '%1'!")
                                     .arg(i + 1).arg(letter));
            return false;
        }
    }
...
void Board::win() {
    end(Icon::load("Win.svg"), tr("You guessed the answer correctly!"));
